...
- süreli ve contract hash'li runtime gateway oturumları ile argümansız karar geçmişi;
...
| T-21 | Gateway oturumunun yetkisiz oluşturulması veya karar geçmişinin okunması | Politika manipülasyonu veya metadata ifşası | Yerel bind varsayımı, süreli oturum ve değişmez contract hash'i | Üretimde kimlik doğrulama, RBAC, tenant ayrımı ve session sahipliği gerekir |
| T-22 | Yetkilendirmeden sonra farklı argümanla araç çalıştırma | Karar ile yan etki arasında TOCTOU | Oturum contract'ı sabittir ve kanonik `ToolCall` SHA-256 özeti `request_hash` olarak karara bağlanır | Adapter aynı özeti çalıştırma anında doğrulamazsa veya gateway'i atlayan yol varsa kontrol etkisiz kalır |
